...
<input type="file" class="file_input" :ref="`stemfile${questionindex}`" :key="`stemfile${questionindex}`" :id="`stemfile${questionindex}`" @change="attacharticlesecfile('stemfile'+questionindex,questionindex)" multiple/></li>
...
this.activeflag=0;
 ...
 removestemimage(qindex,optindex, index){
  ...
  axios.post(`/deletesingleKFPquestion`,{"questionid":this.questions[qindex].id})
    .then(response => {
      // JSON responses are automatically parsed.
...
axios.post(`/updatepublishquestion`,{"q_order":this.questions[0].q_order,"oldq_version":this.oldq_version,"question_version":this.question_version,"id":this.masterquestionuid,"q_subject":this.q_subject,"q_title":this.q_title,"answer_key":this.questions[0].answer_key,"imagevisibility":this.questions[0].imagevisibility,"q_version":this.q_version,"uploadedstem_file":this.questions[0].uploadedstem_file,"uploaded_file":this.questions[0].uploaded_file,"questionimagevalidation":this.questions[0].questionimagevalidation,"stemimagevalidation":this.questions[0].stemimagevalidation,"stem_text":this.questions[0].stem[0].stem_text,"instruction_text":this.questions[0].instruction_text,"version":this.version,"questiontype":this.q_typename,"college":this.college,"question_text":this.questions[0].question_text,"option_type":this.questions[0].choiceoption,"answer_explanation":this.questions[0].answer_explanation,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser})
    ...
    .then(optresponse => {
          ...
          axios.post(`/updatesavekfpquestion`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"q_subject":this.q_subject,"q_title":this.q_title,"q_version":this.q_version,"questiontype":this.q_typename,"college":this.college,"data":this.questions,"version":this.version,'parentqid':this.questionid,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser})
    .then(kfpresponse => {
if(this.publishcheck===0 || this.oldq_version!==this.question_version){
        ...
        .then((postresponse)=>{
           axios.post('/setquestionpublishnotification',{"weburl":'questionanswer',"appurl":'',"targetid":this.questionid,"q_type":this.q_typenoty,"title":this.q_title,"questionid":this.questionid,postid:postresponse.data.unique_id,createdby:this.currentuser,scheduled:this.scheduledatetime})
        .then((notiresponse)=>{
 ...
 created() {
    ...
    .then(response => {
      ...
      this.currentuser = response.data.unique_id
       axios.post(`/getquestionpostdata`,{"questionid":this.$route.query.id,"loginid":this.currentuser})
    .then(postresponse => {
...
console.log(postresponse.data);
    ...
    axios.post(`/getkfpquestionbyid`,{"questionid":this.$route.query.id})
    .then(response => {
      console.log(response);
      var i=0;
...
this.questions[i].optionchoice="radio";
      ...
      var j=0;
      console.log('length'+response.data[i].kfpquestion_options.length)
      console.log(response.data[i].kfpquestion_options)
      if(response.data[i].kfpquestion_options.length > 0){
          this.optioncounts=response.data[i].kfpquestion_options.length
      for(j=0; j < response.data[i].kfpquestion_options.length; j++){
      var tempopt={'id':response.data[i].kfpquestion_options[j]._id,'answervalue':response.data[i].kfpquestion_options[j].answervalue,'option_text':response.data[i].kfpquestion_options[j].option_text,'option_order':response.data[i].kfpquestion_options[j].option_order};
