      ...
      <span v-if="resourcecheck">
        <a  class="container_link" id="secresource" href="#resource" >Resources</a>
            ...
            <ul class="article_social_icons">
               ...
               <li class="cursoron addsummary"><span class="new-question_button"><a :href="`/resourcemodule?id=${masterquestion}`">Add Resources</a></span></li>
...
<button class="btn q_prvw_sbmtbtn" type="button" @click="submitimgweekanswer">Submit</button></div><!--button_previw-->
</div>
</div>
    <!--selection_section-->
...
<button class="button youranswer_btn youranswer_button_wrong">Your Answer</button>
  ...
  <span v-else>
    ...
    <textarea v-model="commentsofpostdata.content" class="add_newcomment text_update" >{{commentsofpostdata.content}}</textarea></span>


     <div class="read_more" v-if="commentsofpostdata.questionid!=null && commentsofpostdata.editflag==false" > <a :href="`/userimagequestionanswer?qid=${masterquestion}&uid=${commentsofpostdata.commentby.unique_id}`" target="_BLANK">Read The Answer</a></div>
  <div class="last_li date_right">{{ commentsofpostdata.created_at | postsumDate}}</div>
    ...
    <div class="replyfield" v-if="replyflag==commentsofpostdata.unique_id">
      ...
      <textarea v-model="replyoncomment" class="replyoncomment text_reply" placeholder="Please reply here" ></textarea>
    </div>
</div>
  <div class="chats second_chats" v-for="(commentsofpost_reply,crindex) in commentsofpostdata.commentofcomment">
    ...
    <div class="chatter">
      ...
      <div v-if="commentsofpost_reply.parentcontent.length >0" class="bg_part">
        ...
        <textarea v-model="commentsofpost_reply.content" class="add_newcomment text_update" >{{commentsofpost_reply.content}}</textarea></span>
      </div>
      <div v-else>
         <!-- <span class="reply_on_comnt_reply" v-if="commentsofpost_reply.reditflag==false" v-html="commentsofpost_reply.content"></span>
        <span v-else>
...
<textarea v-model="feedback" ref="comment_boxrep" class="add_newcomment text_comment" placeholder="Please comment here" ></textarea>
...
<div class="bottom_articles">
  ...
  <div class="col-sm-3" v-for="item in resourceModule" :key="item._id">
    <div class="single_bottom_article">
     ...
     <div v-else-if="item.resources.siteUrl !== null">
  <template v-if="item.resources.linkpreviewdata!==null">
<a :href="item.resources.siteUrl" class="cursoron" target="_blank">
        <div class="recreation" >
          <div class="contents">
            <h4>{{item.resources.linkpreviewdata.title}}</h4>
          </div>
            <div class="image">
              <img v-if="item.resources.linkpreviewdata.imgs.length>0" :src="item.resources.linkpreviewdata.imgs[0]" />
            </div>
          <div class="recreation_content">
...
<template v-else>
      <div class="img_Section" >
       <a :href="item.resources.siteUrl" class="cursoron" target="_blank"> <link-prevue :url="item.resources.siteUrl"></link-prevue></a>
      </div>
</template>
...
<br>
      ...
      <template v-if="rolespermission.indexOf(currentuserdata.role)>=0">
        <div id="summary" v-if="summarycheck">
...
<!-- <div class="commentfield" v-if="replyflag != post_id"> -->
<span class="comm_current_user commemt_user1"><img :src="currentuserdata.profile" class="circle_image"></span>
<textarea v-model="comment" ref="comment_box" class="add_newcomment text_comment" placeholder="Please comment here" ></textarea>
...
export default {
  ...
  methods:
  {
    submitimgweekanswer()
    {
        ...
        if(this.answevalidation.length==0)
        {
          ...
          axios.post(`/imgofweekquestionanswer`,{dummyname:this.dummyname,scurrenttime:this.scurrenttime,title:this.questiondata[0].q_title,anonymouscheck:this.anonymouscheck,questionid:this.masterquestion,createdby:this.currentuser,answerdata:this.questiondata})
          .then(response => {
    ...
    getquestiondata()
    {
        axios.post(`/getimgweekquestionforanswer`,{"questionid":this.masterquestion,"loggedinuser":this.currentuser})
        .then(response =>
        {
    ...
    addComment(commentid)
    {
        ...
        {
          this.freeze=false
          axios.post(`/addimgweekcomments`,{dummyname:this.commentdummyname,title:this.questiondata[0].q_title,asynccheck:this.commentanonymouscheck,comment:this.comment,questionid:this.masterquestion,postid:this.post_id,commentid:commentid,replyid:commentid,createdby:this.currentuser})
          .then(response => {
            this.updateComments();
            this.getquestiondata();
            this.comment=null
    ...
    addFeedback(commentid)
    {
        ...
        {
          axios.post(`/addimgweekcomments`,{dummyname:this.commentdummyname,title:this.questiondata[0].q_title,asynccheck:this.commentanonymouscheck,comment:this.feedback,questionid:this.masterquestion,postid:this.post_id,commentid:commentid,replyid:commentid,createdby:this.currentuser})
          .then(response => {
            this.updateComments();
            this.getquestiondata();
            this.feedback=null
    ...
    addReply(commentofpostid,replyid)
    {
        ...
        console.log(replyid);
        console.log(this.replyoncomment);
  ...
  created()
  {
    ...
    // to fetch data for resource module
