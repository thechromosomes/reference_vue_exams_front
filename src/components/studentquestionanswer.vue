...
<v-container qustionans>
            ...
            <ul class="article_social_icons">
                ...
                <li class="cursoron" v-if="questionLabels[4].rolevalue.indexOf(currentuserdata.role)>=0 || postdata.created_by==currentuser"><span class="new-question_button"><a :href="`/addnewstudentquestion?id=${masterquestion}&flag=new&subid=${questiondata.length}`">Add new question</a></span></li>
 ...
 //      },
    submitcaseanswer()
    {
      ...
      if(this.answevalidation.length==0)
      {
        ...
        axios.post(`/studentquestionanswer`,{dummyname:this.dummyname,scurrenttime:this.scurrenttime,anonymouscheck:this.anonymouscheck,questionid:this.masterquestion,createdby:this.currentuser,answerdata:this.questiondata})
        .then(response =>
        {
          console.log('question submit after');
          this.getquestiondata();
    ...
    getquestiondata()
    {
      axios.post(`/getstudentquestionforanswer`,{"questionid":this.masterquestion,"loginid":this.currentuser})
      .then(response =>
      {
    ...
    removecomment(id)
    {
      axios.post(`/removestudentcomment`,{"id":id})
      .then(removeresponse => {
        this.getquestiondata();
      }).catch(e => {
          this.errors.push(e)
    ...
    updateEditReply(questionindex,commentindex,replyindex,replyid)
    {
      ...
      axios.post(`/updatestudentcomments`,{id:replyid,content:this.editedreply, createdby:this.currentuser})
      .then(response => {
    ...
    {
      await axios.post(`/getstudentcommentlikers`,{'commentid':id})
      .then(response => {
    ...
    addComment(questionindex,questionid)
    {
      ...
      {
      this.freeze = false
        axios.post(`/addstudentcomment`,{'title':this.questiondata[0].q_title,asynccheck:this.anonymouscheck,dummyname:this.dummyname,postid:this.post_id,content:this.comment,questionid:questionid,parentqid:this.masterquestion,commentid:null,replyid:null,createdby:this.currentuser})
        .then(response =>
        {
          this.getquestiondata();
          this.comment=null
    ...
    addReply(questionindex,questionid,commentid,replyid)
    {
      ...
      {
        axios.post(`/addstudentcomment`,{asynccheck:false,dummyname:null,postid:this.post_id,content:this.replyoncomment,questionid:questionid,parentqid:this.masterquestion,commentid:commentid,replyid:replyid,createdby:this.currentuser})
        .then(response => {
          this.getquestiondata();
    ...
    comlikeanddislike(commentid,status)
    {
      ...
      console.log(status);
      axios.post(`/studentlikedislikecomment`,{'commentid':commentid,'likeby':this.currentuser,'status':status})
      .then(response => {
    ...
    likeanddislike(postid,status)
    {
      ...
      .then(response =>
      {
        ...
        if(status==1)
        {
          axios.post('/setpostlikenotification',{"weburl":'studentquestionanswer',"appurl":'',"targetid":this.masterquestion,"q_type":this.questiontype,"title":null,"questionid":this.masterquestion,postid:postid,createdby:this.currentuser})
          .then((notiresponse)=>{
    ...
    updateComment(questionindex,commentindex,id)
    {
      ...
      if(this.comment!=null && this.comment.trim())
      {
        axios.post(`/updatestudentcomments`,{'id':id,content:this.comment,createdby:this.currentuser})
        .then(response => {
          this.getquestiondata();
          this.comment=null
        })
