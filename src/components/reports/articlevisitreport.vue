<template>
  <v-container fluid subtopic_section qus_rprt>
    <v-layout row wrap>
      <v-flex xs12><h2 class="add_title">Article Visit Report</h2></v-flex>
    </v-layout>
    <template>
      <v-card class="clearqstnrprts" >
        <v-card-title>
          Article Visit Report
          <v-spacer></v-spacer>
          <v-text-field
            v-model="search"
            append-icon="search"
 ...
 created() {
    ...
    .catch(e => {
      this.errors.push(e)
          ...
          axios.post(`/getarticleview`,{"articleid":this.$route.query.id})
    .then(response => {
      // JSON responses are automatically parsed.
      this.articleviews = response.data
    })
    .catch(e => {
      this.errors.push(e)
    })
