      ...
      <v-card class="clearqstnrprts" >
        ...
        >
          ...
          <template  slot="items" slot-scope="props"> 
             ...
             <td>{{ commentcount }}</td>
             <td> <a  :href="`/userimagequestionanswer?qid=${masterquestion}&uid=${props.item.submitbyuser.unique_id}`" class="answer_link">View</a></td>
          </template>
...
export default {
  ...
  created() {
    ...
    axios.post(`/getimgweekanswersofquestion`,{"questionid":this.$route.query.id})
    .then(response => {
