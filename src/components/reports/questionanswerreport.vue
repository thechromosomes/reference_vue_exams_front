<template>
  <v-container fluid subtopic_section qus_rprt>
    <v-layout row wrap>
      <v-flex xs12><h2 class="add_title">Question Answer Report</h2></v-flex>
    </v-layout>
    <template>
        ...
        >
          <template  slot="items" slot-scope="props">
            <td>{{ props.item.submitbyuser.firstname }}</td>
             ...
             <td>{{props.item.rightanswer*100/props.item.optioncount}}%</td>
          </template>
    ...
    data() {
      return {
        ...
        headers: [
          ...
          { text: 'Answer Status', sortable: true, value:'rightanswer' }
 ...
 created() {
    ...
    .catch(e => {
      this.errors.push(e)
          ...
          axios.post(`/getanswersofquestion`,{"questionid":this.$route.query.id})
    .then(response => {
      // JSON responses are automatically parsed.
