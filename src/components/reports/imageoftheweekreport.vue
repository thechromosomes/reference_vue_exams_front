<template>
  <v-container fluid subtopic_section qus_rprt>
    <v-layout row wrap>
      <v-flex xs12><h2 class="add_title">Image of the week Question Bank Report</h2></v-flex>
    </v-layout>
      ...
      <v-card class="clearqstnrprts last_two_hide" >
        ...
        >
          <template  slot="items" slot-scope="props">
            ...
            <td> <a  :href="`/imagequestionanswerreport?id=${props.item._id}`" class="answer_link">{{props.item.answercount.length}}</a></td>
            <td> <a  :href="`/imageoftheweekvisitreport?id=${props.item._id}`" class="answer_link">{{props.item.visitcount.length}}</a></td>
            <td>
              <span class="action_group">
                <a :href="`/imageofweekanswer?id=${props.item._id}`" target="_blank" title="View"><img src="../../assets/icons/eye.svg" class="fa_svg_icon" /></a> 
                <a :href="`/imageeditquestion?id=${props.item._id}`"><img src="../../assets/icons/edit.svg" class="fa_svg_icon" /></a>  
...
export default {
  ...
  data() {
    return {
      ...
      errors: [],
      questiontypes:[],
      q_typename:'5d15fea98edfed6c417592d15',
      ...
      headers: [
          ...
          { text: 'Actions', sortable: false, value:'createdfilter' },
  ...
  methods: {
    ...
    deleteQuestion(questionid)
    {
      ...
      {
        axios.post(`/deleteimgweekquestion`,{"questionid":questionid})
        .then(response => 
        {
