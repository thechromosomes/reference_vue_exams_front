      ...
      <v-card class="clearqstnrprts" >
        ...
        >
          ...
          <template  slot="items" slot-scope="props"> 
             ...
             <td>{{ commentcount }}</td>
             <td> <a  :href="`/userquestionanswer?qid=${masterquestion}&uid=${props.item.submitbyuser.unique_id}`" class="answer_link">View</a></td>
          </template>
...
import axios from 'axios';
 ...
 created() {
   this.masterquestion=this.$route.query.id;
    ...
    axios.post(`/getkfpanswersofquestion`,{"questionid":this.$route.query.id})
    .then(response => {
      // JSON responses are automatically parsed.
