<template>
  <v-container fluid subtopic_section qus_rprt>
    <v-layout row wrap>
      <v-flex xs12><h2 class="add_title">KFP Answer Report</h2></v-flex>
    </v-layout>
...
import axios from 'axios';
  export default {
    data() {
      return {
        ...
        headers: [
          ...
          { text: 'Comments', sortable: false, value:'commentcount' },
          { value:'createdfilter' },
          { value:'datafilter' }
        ],
        questions: []
    ...
    methods: {
updateoptioncount(){
  axios.post(`/getkfpuserwiseanswers`,{"userid":this.submitter})
    .then(response => {
...
deleteQuestion(questionid){
 ...
 created() {
    ...
    .then(response => {
      ...
      axios.post(`/getkfpuserwiseanswers`,{"userid":this.currentuser})
    .then(uresponse => {
      // JSON responses are automatically parsed.
    ...
    var i=0;
      for(i=0; i<=uresponse.data.length; i++){
        uresponse.data[i].createdfilter=dateFormat(uresponse.data[i].questiondata.created_at,"dd/mm/yyyy");
        uresponse.data[i].datafilter=dateFormat(uresponse.data[i].created_at,"dd/mm/yyyy");
