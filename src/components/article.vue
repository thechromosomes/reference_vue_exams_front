...
<p class="left_heading col-md-12"><img src="../assets/icons/question-square-solid_green.svg" class="ques_icon" /> Create Article</p>
...
<div class="question_fields col-md-12">
<tinymce v-bind:id="`article_content${articleindex}`" v-model="article.article_content" v-bind:key="`article_content${articleindex}`" :other_options="spelloptions"></tinymce>
</div>
...
<div class="question_fields col-md-12" v-if="article.article_text_flag">
...
<tinymce v-bind:id="`article_text${articleindex}`" v-model="article.article_text" v-bind:key="`article_text${articleindex}`" :other_options="spelloptions"></tinymce>
</div>
 <div class="choices_fields col-md-12"> 
...
<tinymce v-bind:id="`question_content${articleindex}${questionindex}`" v-model="question.question_text" v-bind:key="`question_content${articleindex}${questionindex}`" :other_options="spelloptions"></tinymce>
</div>
...
<tinymce v-bind:id="`answer_explanation${articleindex}${questionindex}`" v-bind:key="`answer_explanation${articleindex}${questionindex}`" v-model="question.answer_explanation" :other_options="spelloptions"></tinymce>
</div>
...
<li style="position: relative;"><span class="right_icon"><img src="../assets/icons/plus-circle_white.svg" class="ins_icon" /></span>Add Image <input type="file" :ref="`file${articleindex}`" :key="`file${articleindex}`" :id="`file${articleindex}`" @change="attachfiletopost('file'+articleindex,articleindex)" multiple/></li>
...
<div class="col-md-12">
<div class="col-md-7 schedule_main"> <span class="add_qschedule_p cursoron" @click="addscheduler=true"> <img src="../assets/icons/alarm-clock.svg" class="fa_svg_icon1"/> <span class="time_left" v-if="publishdate > Date.now()">{{publishdate | timeleft}}</span></span><span class="remove_scheduler cursoron" v-if="publishdate > Date.now()" @click="removeScheduler">x</span></span></div>
<template v-if="publishdate <= Date.now()">
<button class="save_fields btn" @click="saveArticle(1)">Publish</button>
</template>
<button class="save_fields btn" @click="saveArticle(0)">Save</button>
</div><!----save_fields---->
</div>
</div>
...
import axios from 'axios';
  export default {
   ...
   data() {
    return {
      articles:[{'article_content':'','article_text':'','article_text_flag':false,'article_section':'','questions':[],'uploaded_file':[],'a_order':1,'section_uploaded_file':[],'sectionimagevalidation':{'image_permission':null,'image_source':null},'mainimagevalidation':{'image_permission':null,'image_source':null}}],
	    author_name:'',
      a_version:1,
      currentuser:null,
       addscheduler:false,
...
this.articles[aindex].questions[qindex].options.splice(id, 1)
      ...
      saveArticle(savestatus)
      {
        ...
        axios.post(`/savearticle`,{"savestatus":savestatus,"a_version":this.a_version,"a_title":this.a_title,"author_name":this.author_name,"tags":tags,"scheduledatetime":this.scheduledatetime,"createdby":this.currentuser,"data":this.articles})
        .then(response => 
        {
          console.log(response);  
          if(savestatus==1)
          {
            axios.post('/addpostbyweb',{"articleid":response.data._id,attachement:null,tags:tags,searchcontent:this.a_title,postcontent:null,createdby:this.currentuser,scheduledatetime:this.scheduledatetime})
            .then((postresponse)=>
            {
              axios.post('/setarticlepublishnotification',{"weburl":'articleview',"appurl":'',"targetid":response.data._id,"title":this.a_title,"articleid":response.data._id,"postid":postresponse.data.unique_id,"authorid":this.author_name,"createdby":this.currentuser,scheduled:this.scheduledatetime})
              .then((notiresponse)=>
              {
