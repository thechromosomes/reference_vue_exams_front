...
<!-- <div class="question_fields col-md-12" v-else-if="question.uploadedstem_file!==null || question.uploadedstem_file.length>0">
   <div class="overlay_hover_image">
        <div class="overlay-image">
            <div class="trashes_icon">
...
<li v-if="kfpflag" @click="addStemp(questionindex)"><span class="right_icon"><img src="../assets/icons/plus-circle_white.svg" class="ins_icon" /></span>Add Aditional STEM</li>
...
<tinymce v-bind:id="`question_text${questionindex}`" v-model="question.question_text" v-bind:key="`question_text${questionindex}`" :ref="`question_text${questionindex}`" @editorInit="editorInitwithtext(question.question_text, 'question_text'+questionindex)" :other_options="spelloptions"></tinymce>
</div>
...
<!-- <div class="question_fields col-md-12" v-else-if="question.uploaded_file !== null || question.uploaded_file.length>0">
   <div class="overlay_hover_image">
        <div class="overlay-image">
            <div class="trashes_icon">
...
<tinymce v-bind:id="`answer_key${questionindex}`" v-bind:key="`answer_key${questionindex}`" v-model="question.answer_key" :ref="`answer_key${questionindex}`" @editorInit="editorInitwithtext(question.answer_key, 'answer_key'+questionindex)" :other_options="spelloptions"></tinymce>
</div>
...
<tinymce v-bind:id="`answer_explanation${questionindex}`" v-bind:key="`answer_explanation${questionindex}`" v-model="question.answer_explanation" :ref="`answer_explanation${questionindex}`" @editorInit="editorInitwithtext(question.answer_explanation, 'answer_explanation'+questionindex)" :other_options="spelloptions"></tinymce>
...
<template v-if="domains.length>0" v-for="(domain, di) in domains" >
...
<template v-if="icpc2s.length>0" v-for="(icpc2, ici) in icpc2s" >
...
<template v-if="ageranges.length>0" v-for="(agerange, agei) in ageranges" >
...
<template v-if="genders.length>0" v-for="(gender, gi) in genders" >
...
<template  v-if="atsis.length>0" v-for="(atsi, ati) in atsis" >
...
<div v-if="publishstatus==0" class="col-md-7 schedule_main">
...
<!-- <button class="save_fields btn" @click="publishquestion" v-if="publishcheck==0 || oldq_version!==question_version">Publish</button>
...
import axios from 'axios';
  export default {
   ...
   data() {
    return {
      questions:[{'stem':[{'stem_text':''}],'options':[],'question_text':'','optionchoice':'radio','choiceoption':'1','imagevisibility':false,'instruction_flag':true,'instruction_text':'','answer_key':'','answer_explanation':'','uploaded_file':null}],
      questioncount:1,
      updatecheck:0,
      stemcount:1,
      currentuser:null,
 ...
 set_scheduler(){
   ...
   removequestionimage(qindex,optindex, index){
      this.updatecheck=Math.random();
      if (this.questions[qindex].multiple_uploaded_file.length == 1){
        this.questions[qindex].uploaded_file = null
      }
      this.questions[qindex].multiple_uploaded_file.splice(index, 1)
...
axios.post(`/updatepublishquestion`,{"q_order":this.questions[0].q_order,"oldq_version":this.oldq_version,"question_version":this.question_version,"id":this.questions[0].id,"q_subject":this.q_subject,"q_title":this.q_title,"answer_key":this.questions[0].answer_key,"imagevisibility":this.questions[0].imagevisibility,"q_version":this.q_version,"uploadedstem_file":this.questions[0].uploadedstem_file,"uploaded_file":this.questions[0].uploaded_file,"questionimagevalidation":this.questions[0].questionimagevalidation,"stemimagevalidation":this.questions[0].stemimagevalidation,"stem_text":this.questions[0].stem[0].stem_text,"instruction_text":this.questions[0].instruction_text,"version":this.version,"questiontype":this.q_typename,"college":this.college,"question_text":this.questions[0].question_text,"option_type":this.questions[0].choiceoption,"answer_explanation":this.questions[0].answer_explanation,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser, "multiple_uploadedstem_file":this.questions[0].multiple_uploadedstem_file,"multiple_uploaded_file":this.questions[0].multiple_uploaded_file})
    ...
    .then(optresponse => {
      if(this.publishcheck===0 || this.oldq_version!==this.question_version){
        ...
        .then((postresponse)=>{
           axios.post('/setquestionpublishnotification',{"weburl":'sbaquestionanswer',"appurl":'',"targetid":this.questionid,"q_type":this.q_typenoty,"title":this.q_title,"questionid":this.questionid,postid:postresponse.data.unique_id,createdby:this.currentuser,scheduled:this.scheduledatetime})
        .then((notiresponse)=>{
 ...
 location.href='/questionreport';
   ...
   //location.reload();
      ...
      savequestion()
      {
        ...
        axios.post(`/updatesavequestion`,{"q_order":this.questions[0].q_order,"oldq_version":this.oldq_version,"question_version":this.question_version,"id":this.questions[0].id,"q_subject":this.q_subject,"q_title":this.q_title,"answer_key":this.questions[0].answer_key,"imagevisibility":this.questions[0].imagevisibility,"q_version":this.q_version,"uploadedstem_file":this.questions[0].uploadedstem_file,"uploaded_file":this.questions[0].uploaded_file,"questionimagevalidation":this.questions[0].questionimagevalidation,"stemimagevalidation":this.questions[0].stemimagevalidation,"stem_text":this.questions[0].stem[0].stem_text,"instruction_text":this.questions[0].instruction_text,"version":this.version,"questiontype":this.q_typename,"college":this.college,"question_text":this.questions[0].question_text,"option_type":this.questions[0].choiceoption,"answer_explanation":this.questions[0].answer_explanation,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser, "multiple_uploadedstem_file":this.questions[0].multiple_uploadedstem_file,"multiple_uploaded_file":this.questions[0].multiple_uploaded_file})
    .then(response => {
      if(response.data.error)
      {
          this.showerror=true;
...
this.questions.push({'stem':[],'options':[],'question_text':'','optionchoice':'radio','choiceoption':'1','defaultimg':'0','instruction_flag':false,'instruction_text':'','answer_key':'','answer_explanation':'','uploaded_file':null,'uploadedstem_file':null,'stemimagevalidation':{'image_permission':null,'image_source':null},'questionimagevalidation':{'image_permission':null,'image_source':null},'imagevisibility':false,'q_order':qorder})
 ...
 created() {
    ...
    axios.post(`/getquestionbyid`,{"questionid":this.$route.query.id})
    .then(response => {
      console.log(response);
      var data=response.data[0];
      // JSON responses are automatically parsed.
      this.questions[0].question_text = data.question_text
       if(data.instruction_text){
        this.questions[0].instruction_flag=true
      }else{
       this.questions[0].instruction_flag=false 
      }
      this.questions[0].id = data._id
      this.questions[0].instruction_text = data.instruction_text
      this.questions[0].answer_explanation = data.answer_explanation
      this.questions[0].answer_key = data.answer_key
      this.questions[0].stem[0].stem_text = data.stem_text
      this.questions[0].uploaded_file = data.hero_img
      this.questions[0].uploadedstem_file = data.uploadedstem_file
      this.questions[0].questionimagevalidation = data.questionimagevalidation
      this.questions[0].stemimagevalidation = data.stemimagevalidation
      this.questions[0].q_order = data.q_order

        // my new line to add multiple image
         if (response.data[0].multiple_uploadedstem_file && response.data[0].multiple_uploaded_file){
          this.questions[0].multiple_uploadedstem_file = response.data[0].multiple_uploadedstem_file
          this.questions[0].multiple_uploaded_file = response.data[0].multiple_uploaded_file
         }else{
          this.questions[0].multiple_uploadedstem_file = []
          this.questions[0].multiple_uploaded_file = []
         }

      this.q_typename=data.questiontype
      this.publishdate=data.publish_date
        this.scheduledatetime=data.publish_date
      ...
      this.q_subject=data.q_subject
      this.q_version=data.q_version
      this.oldq_version=data.q_version
      this.newq_version=data.q_version
      this.newq_version++
      this.question_version=data.q_version
      this.version=data.version
      this.publishcheck=data.publish
      this.questions[0].choiceoption=data.option_type
      if(this.questions[0].choiceoption==2){
      this.questions[0].optionchoice="checkbox";
      }else{
this.questions[0].optionchoice="radio";
}
      var j=0;
      if(data.question_options.length >=0){
          this.optioncounts=data.question_options.length
      for(j = 0; j < this.optioncounts; j++){
      var tempopt={'id':data.question_options[j]._id,'answervalue':data.question_options[j].answervalue,'option_text':data.question_options[j].option_text,'option_order':data.question_options[j].option_order}
            this.questions[0].options.push(tempopt);
      }
    }
if(data.tagdata.length >=0){
 for(j=0;j<data.tagdata.length;j++){
      this.selected_tags.push(data.tagdata[j]);
      this.selected_tags_id.push(data.tagdata[j].unique_id);
      }
}  
if(data.domaindata.length >=0){
      for(j=0;j<data.domaindata.length;j++){
      this.selected_domains.push(data.domaindata[j]);
      this.selected_domains_id.push(data.domaindata[j].unique_id);
 } }
if(data.categorydata.length >=0){
      for(j=0;j<data.categorydata.length;j++){
      this.selected_icpc2s.push(data.categorydata[j]);
      this.selected_icpc2s_id.push(data.categorydata[j].unique_id);
      }
}
if(data.age_rangedata.length >= 0){
      for(j=0;j<data.age_rangedata.length;j++){
      this.selected_ages.push(data.age_rangedata[j]);
      this.selected_ages_id.push(data.age_rangedata[j].unique_id);
      } 
}
if(data.genderdata.length >= 0){
      for(j=0;j<data.genderdata.length;j++){
      this.selected_genders.push(data.genderdata[j]);
      this.selected_genders_id.push(data.genderdata[j].unique_id);
      } 
}
if(response.data[0].atsi){
  ...
  computed: {
    ...
    ftags() {
      return this.domains.filter((domain) => domain.distance <= 15)
    }
  }
