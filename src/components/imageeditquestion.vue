...
<div class="question_fields col-md-12">
<label v-if="imgofweekflag">Stem {{questionindex+1}}</label>
<label v-if="!imgofweekflag">Stem </label>
...
<div class="question_fields col-md-12" v-if="imgofweekflag">
...
<li class="remove_image_section" @click="removeQuestion(questionindex)"> <div class="spa_circle"> <span class="minus_image"> - </span> </div> Remove question </li>
</ul>
</div>
<div class="question_fields col-md-12">
<label v-if="imgofweekflag">Question Text {{questionindex+1}}</label>
<label v-if="!imgofweekflag">Question Text </label>
...
<label v-if="imgofweekflag">Instructions {{questionindex+1}}</label>
<label v-if="!imgofweekflag">Instructions </label>
...
<div class="question_fields col-md-12" v-if="imgofweekflag">
<label v-if="imgofweekflag">Answer key {{questionindex+1}}</label>
<label v-if="!imgofweekflag">Answer key </label>
...
<label v-if="imgofweekflag" >Answer explanation {{questionindex+1}}</label>
<label v-if="!imgofweekflag" >Answer explanation </label>
...
<div class="col-md-12">
<div class="col-sm-6 schedule_main" v-if="publishstatus==0"> 
...
<template v-if="publishdate <= Date.now()">
 ...
 <v-text-field
      ...
      <v-dialog v-model="imagevalidationpopup" width="600" persistent>
...
export default {
  ...
  methods: 
  {
    ...
    searchtags()
    {
      axios.post(`/gettags`,{'searchedtag':this.m_tags}).then(response => 
      {
    ...
    removequestionimage(qindex,stemindex, index)
    {
      this.updatecheck=Math.random();
    ...
    removeQuestion(qindex)
    {
      ...
      {
        if(this.questions[qindex].id){     
          axios.post(`/deletesingleimgweekquestion`,{"questionid":this.questions[qindex].id}).then(response => 
          {
...
this.questions[this.questionindexid].uploadedstem_file=null
    ...
    publishquestion()
    {
      ...
      axios.post(`/updatepublishquestion`,{"q_order":this.questions[0].q_order,"oldq_version":this.oldq_version,"question_version":this.question_version,"id":this.masterquestionuid,"q_subject":this.q_subject,"q_title":this.q_title,"answer_key":this.questions[0].answer_key,"imagevisibility":this.questions[0].imagevisibility,"q_version":this.q_version,"uploadedstem_file":this.questions[0].uploadedstem_file,"uploaded_file":this.questions[0].uploaded_file,"questionimagevalidation":this.questions[0].questionimagevalidation,"stemimagevalidation":this.questions[0].stemimagevalidation,"stem_text":this.questions[0].stem[0].stem_text,"instruction_text":this.questions[0].instruction_text,"version":this.version,"questiontype":this.q_typename,"college":this.college,"question_text":this.questions[0].question_text,"option_type":this.questions[0].choiceoption,"answer_explanation":this.questions[0].answer_explanation,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser}).then(response => 
      {
        this.questionid=response.data._id;
        axios.post(`/updatesavequestionoption`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"questionid":response.data._id,"createdby":this.currentuser,"optiondatas":this.questions[0].options}).then(optresponse => 
        {
          if(this.q_typename=='5d15fea98edfed6c417592d15')
          {
            axios.post(`/updatesaveimgweekquestion`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"q_subject":this.q_subject,"q_title":this.q_title,"q_version":this.q_version,"questiontype":this.q_typename,"college":this.college,"data":this.questions,"version":this.version,'parentqid':this.questionid,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser})
            .then(kfpresponse => 
            {
              if(this.publishcheck===0 || this.oldq_version!==this.question_version)
              {
                axios.post('/addpostbyweb',{"questionid":this.questionid,attachement:null,"questiontype":this.q_typename,tags:tags,postcontent:null,createdby:this.currentuser,scheduledatetime:this.scheduledatetime}).then((postresponse)=>
                {
               axios.post('/setquestionpublishnotification',{"weburl":'imageofweekanswer',"appurl":'',"targetid":this.questionid,"q_type":this.q_typenoty,"title":this.q_title,"questionid":this.questionid,postid:postresponse.data.unique_id,createdby:this.currentuser,scheduled:this.scheduledatetime})
            .then((notiresponse)=>{
    ...
    {   
      ...
      .then(response => 
      {
        ...
        axios.post(`/updatesavequestionoption`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"questionid":response.data._id,"createdby":this.currentuser,"optiondatas":this.questions[0].options}).then(optresponse => 
        {
          if(this.q_typename=='5d15fea98edfed6c417592d15')
          {
            axios.post(`/updatesaveimgweekquestion`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"q_subject":this.q_subject,"q_title":this.q_title,"q_version":this.q_version,"questiontype":this.q_typename,"college":this.college,"data":this.questions,"version":this.version,'parentqid':this.questionid,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser}).then(kfpresponse => 
            {
              ...
              if(this.scheduledatetime>Date.now() && this.oldq_version!==this.question_version)
              {
                axios.post('/addpostbyweb',{"questionid":this.questionid,attachement:null,"questiontype":this.q_typename,tags:tags,postcontent:null,createdby:this.currentuser,scheduledatetime:this.scheduledatetime}).then((postresponse)=>
                {
                  //            axios.post('/setquestionpublishnotification',{"weburl":'caseofweekanswer',"appurl":'',"targetid":this.questionid,"q_type":this.q_typenoty,"title":this.q_title,"questionid":this.questionid,postid:postresponse.data.unique_id,createdby:this.currentuser})
                  //         .then((notiresponse)=>{
                  // console.log(notiresponse);
                  // location.reload();
                  location.href='/imageoftheweekreport';
  ...
  created() 
  {
    axios.post(`/getcurrentuser`,{'username':username}).then(response => 
    {
      ...
      axios.post(`/getquestionpostdata`,{"questionid":this.$route.query.id,"loginid":this.currentuser}).then(postresponse => 
      {
    ...
    axios.get(`/getallquestiontypes`).then(response => 
    {
      this.questiontypes = response.data
    }).catch(e => {
        this.errors.push(e)
      }),
    axios.get(`/getallcolleges`).then(response => 
    {
      this.colleges = response.data
    }).catch(e => {
        this.errors.push(e)
      }),
    axios.get(`/getalldomain`).then(response => 
    {
      this.domains = response.data
    }).catch(e => {
        this.errors.push(e)
      }),
    axios.get(`/getallicpc2`).then(response => 
    {
      this.icpc2s = response.data
    }).catch(e => {
        this.errors.push(e)
      }),
    axios.get(`/getallgender`).then(response => 
    {
      this.genders = response.data
    }).catch(e => {
        this.errors.push(e)
      }),
    axios.get(`/getallagerange`).then(response => 
    {
      this.ageranges = response.data
    }).catch(e => {
        this.errors.push(e)
    ...
    axios.post(`/getpostquestionstatus`,{"quesid":this.$route.query.id}).then(responsestatus => 
    {
    ...
    axios.post(`/getimgweekquestionbyid`,{"questionid":this.$route.query.id}).then(response => 
    {
      console.log(response);
      var i=0;
      ...
      for(i=0; i < response.data.length; i++)
      {
        ...
        var j=0;
        console.log('length'+response.data[i].imgofweekquestion_options.length)
        console.log(response.data[i].imgofweekquestion_options)
        if(response.data[i].imgofweekquestion_options.length > 0)
        {
          this.optioncounts=response.data[i].imgofweekquestion_options.length
          for(j=0; j < response.data[i].imgofweekquestion_options.length; j++)
          {
            var tempopt={'id':response.data[i].imgofweekquestion_options[j]._id,'answervalue':response.data[i].imgofweekquestion_options[j].answervalue,'option_text':response.data[i].imgofweekquestion_options[j].option_text,'option_order':response.data[i].imgofweekquestion_options[j].option_order};
