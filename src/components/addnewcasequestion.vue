...
<div v-for="(question,questionindex) in questions" class="question_section" v-bind:id="`${question.id}`" v-if="questionindex==subid">
<div class="row">
...
<div class="question_fields col-md-12" v-if="question.uploadedstem_file!==null">
...
<input type="file" class="file_input" :ref="`stemfile${questionindex}`" :key="`stemfile${questionindex}`" :id="`stemfile${questionindex}`" @change="attachfilestemfile('stemfile'+questionindex,questionindex)"/></li>
</ul>
</div>
<div class="question_fields col-md-12">
...
<div class="question_fields col-md-12" v-if="question.uploaded_file">
   <div class="overlay_hover_image">
        <div class="overlay-image">
            <div class="trashes_icon">
...
<li ><span class="right_icon"><img src="../assets/icons/plus-circle_white.svg" class="ins_icon" /></span>Add Image <input type="file" :ref="`file${questionindex}`" :key="`file${questionindex}`" :id="`file${questionindex}`" @change="attachfiletopost('file'+questionindex,questionindex)"/></li>
...
<div class="col-md-12">
<!-- <div class="col-sm-6 schedule_main"> <span class="add_qschedule_p cursoron" @click="addscheduler=true"> <img src="../assets/icons/alarm-clock.svg" class="fa_svg_icon1"/> </span>
...
this.activeflag=0;
        ...
        removestemimage(qindex,optindex){
...
this.questions[qindex].instruction_flag=true
    ...
    if(i!=indexid){
 this.optiondatas[i].answervalue=false
    }
     }
      }
     ...
     publishquestion(){
        ...
        if(this.questions[quesindex].question_text==''){
          this.questions.splice(quesindex, 1)
           ...
           if(this.q_typename=='5d15fea98edfed6c417592d14'){
          axios.post(`/updatesavecasebasequestion`,{"oldq_version":this.oldq_version,"question_version":this.question_version,"q_subject":this.q_subject,"q_title":this.q_title,"q_version":this.q_version,"questiontype":this.q_typename,"college":this.college,"data":this.questions,"version":this.version,'parentqid':this.questionid,"tags":tags,"domain":domain,"category":category,"age_range":age_range,"gender":gender,"atsi":atsi,"deleted":"0","publish_date":this.scheduledatetime,"answerpublish_date":this.scheduledatetime,"createdby":this.currentuser})
    .then(kfpresponse => {
if(this.publishcheck===0 || this.oldq_version!==this.question_version){
  location.href='/caseofweekanswer?id='+this.masterquestionuid;
...
this.questions.push({'stem':[],'options':[],'question_text':'','optionchoice':'radio','choiceoption':'1','defaultimg':'0','instruction_flag':false,'instruction_text':'','answer_key':'','answer_explanation':'','uploaded_file':null,"uploadedstem_file": null,'imagevisibility':false,'q_order':qorder})
 ...
 created() {
    ...
    axios.post(`/getcasequestionbyid`,{"questionid":this.$route.query.id})
    .then(response => {
      console.log(response);
      var i=0;
      ...
      for(i=0; i < response.data.length; i++){
       this.questions[i]= {'stem':[{'stem_text':''}],'options':[],'question_text':'','optionchoice':'radio','choiceoption':'1','imagevisibility':false,'instruction_flag':false,'stemimagevalidation':{'image_permission':null,'image_source':null},'questionimagevalidation':{'image_permission':null,'image_source':null},'instruction_text':'','answer_key':'','answer_explanation':'','uploaded_file':null};
...
this.questions[i].optionchoice="radio";
      ...
      var j=0;
      console.log('length'+response.data[i].casequestion_options.length)
      console.log(response.data[i].casequestion_options)
      if(response.data[i].casequestion_options.length > 0){
          this.optioncounts=response.data[i].casequestion_options.length
      for(j=0; j < response.data[i].casequestion_options.length; j++){
      var tempopt={'id':response.data[i].casequestion_options[j]._id,'answervalue':response.data[i].casequestion_options[j].answervalue,'option_text':response.data[i].casequestion_options[j].option_text,'option_order':response.data[i].casequestion_options[j].option_order};
...
if(this.$route.query.flag=='new'){
          ...
          console.log(this.$refs);
this.questions.push({'stem':[],'options':[],'question_text':'','optionchoice':'radio','choiceoption':'1','defaultimg':'0','instruction_flag':false,'stemimagevalidation':{'image_permission':null,'image_source':null},'questionimagevalidation':{'image_permission':null,'image_source':null},'instruction_text':'','answer_key':'','answer_explanation':'','uploaded_file':null,"uploadedstem_file": null,'imagevisibility':false,'q_order':qorder})
